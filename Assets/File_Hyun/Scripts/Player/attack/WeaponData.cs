...
public struct ComboInfo
{
    ...
    public float Combodelay; // 각 콤보가 끝난 뒤 다음 콤보까지 공격 쿨타임(공격속도, 이 시가동안 움직임 불가능, 대시만 가능(캔슬 개념))
    public float ComboKeep; // 각 콤보가 끝난 뒤 다음 콤보를 누르기까지 유지될 수 있는 시간 (콤보가 끝난 뒤 측정 시작, 측정 시작 시 움직이기 가능)
    public GameObject projectilePrefab; // 투사체 프리팹 (Bow, Bomb 전용, 없다면 비워놓기)
}    
...
public class WeaponData : ScriptableObject
{
    ...
    [Header("특수공격 정보")]
    public float skillDamage; // 특수공격 데미지
    public float skillcooldown; // 특수공격 쿨타임
